...
%\newpage
%\section{Next time...}
%In the next class we will implement this abstract model in a \texttt{Jupyter} notebook using \texttt{Pyomo}, and solve it using \texttt{GLPK}.  Before the next class, make sure that you have \texttt{Python}, \texttt{Pyomo}, \texttt{GLPK}, and \texttt{Jupyter} on your computer.  You used this set-up for SA305 and/or SM286D.  If you need to install or reinstall any of these, see the instructions provided on blackboard.  (\emph{Be careful not to install a second version of} \texttt{Anaconda}\emph{, because this can create problems.}  If you need to reinstall, uninstall everything first.)
